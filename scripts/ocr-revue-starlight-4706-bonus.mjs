...
const MACHINE_ID = 4706;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `revue_starlight_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(
  TMP_DIR,
  `revue_starlight_${MACHINE_ID}_bonus_upscaled.jpg`,
);
const OUT_PATH = path.join(TMP_DIR, `ocr_revue_starlight_${MACHINE_ID}_bonus.txt`);
