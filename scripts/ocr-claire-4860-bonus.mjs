...
const MACHINE_ID = 4860;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `claire_${MACHINE_ID}_bonus.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_claire_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  try {
    ...
    for (const psm of psms) {
      ...
      logParts.push("\n- Lines (filtered):");
      logParts.push(lines.slice(0, 120).join("\n"));
      logParts.push("\n- Denominator-like tokens (first 200):");
      logParts.push(denominators.slice(0, 200).join(", "));
      logParts.push("\n- Percent-like tokens:");
