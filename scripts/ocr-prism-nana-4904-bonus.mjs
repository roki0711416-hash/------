...
const MACHINE_ID = 4904;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `prism_nana_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `prism_nana_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_prism_nana_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  try {
    // Upscale for OCR (macOS built-in). Bonus tables often have small fonts.
  ...
  // Pass 1: digits-only OCR (to pick up denominators and payout rates)
  ...
  // Pass 2: Japanese OCR (to capture headers like 初当り / CZ / ST / 出玉率)
