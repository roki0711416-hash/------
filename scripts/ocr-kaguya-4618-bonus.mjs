...
const MACHINE_ID = 4618;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `kaguya_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `kaguya_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_kaguya_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  try {
    ...
    for (const psm of psms) {
      ...
      logParts.push("\n- Lines (filtered):");
      logParts.push(lines.slice(0, 360).join("\n"));
      logParts.push("\n- Numeric-like tokens (first 560):");
      logParts.push(numericLikeTokens.slice(0, 560).join(", "));
      logParts.push("\n- Percent-like tokens:");
