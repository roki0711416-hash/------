...
const MACHINE_ID = 4921;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `fujiko_bt_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `fujiko_bt_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_fujiko_bt_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  try {
    ...
    for (const psm of psms) {
      ...
      logParts.push(denominators.slice(0, 420).join(", "));
      logParts.push("\n- Percent-like tokens:");
