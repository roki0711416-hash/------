...
const MACHINE_ID = 4877;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `zenigata_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `zenigata_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_zenigata_${MACHINE_ID}_bonus.txt`);
...
function extractCandidates(text) {
  ...
  for (const l of lines) {
    // pick up numeric tokens like 1234.5 and also 1/1234.5 style
    for (const m of l.matchAll(/(?:^|\D)(\d{2,6}(?:\.\d)?)(?:$|\D)/g)) {
...
async function main() {
  ...
  // Upscale for OCR (macOS built-in).
  ...
  try {
    ...
    for (const psm of psms) {
      ...
      logParts.push("\n- Lines (filtered):");
      logParts.push(lines.slice(0, 200).join("\n"));
      logParts.push("\n- Numeric-like tokens (first 320):");
      logParts.push(denominators.slice(0, 320).join(", "));
      logParts.push("\n- Percent-like tokens:");
