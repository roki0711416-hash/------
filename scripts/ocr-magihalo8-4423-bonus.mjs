...
const MACHINE_ID = 4423;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `magihalo8_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `magihalo8_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_magihalo8_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  // Pass 1: digits-only OCR (for extracting denominators / rates)
  ...
  // Pass 2: Japanese OCR (for capturing headers like BIG/REG/合算/出玉率)
