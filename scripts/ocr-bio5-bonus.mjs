...
const IMAGE_URL = "https://p-town.dmm.com/upload/machines/4754/bonus.jpg";
const OUT_DIR = path.resolve("scripts", ".tmp");
const OUT_IMAGE_PATH = path.join(OUT_DIR, "bio5_bonus.jpg");
...
function extractNumbers(text) {
	...
	for (const line of lines) {
		...
		for (const m of line.matchAll(/1\s*\/\s*(\d{2,4}\.\d)/g)) {
...
async function main() {
	...
	for (const lang of langsToTry) {
		// Note: On newer Node versions, passing functions into the worker options can
		// trigger DataCloneError (tesseract.js spawns a worker thread internally).
		// Keep it simple and log after recognition instead.
