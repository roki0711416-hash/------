...
const MACHINE_ID = 4360;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `karakuri_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `karakuri_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_karakuri_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  try {
    ...
    for (const psm of psms) {
      ...
      logParts.push("- Raw text (first 2200 chars):");
      logParts.push(text.slice(0, 2200));
      logParts.push("\n- Lines (filtered):");
      logParts.push(lines.slice(0, 420).join("\n"));
      logParts.push("\n- Numeric-like tokens (first 720):");
      logParts.push(numericLikeTokens.slice(0, 720).join(", "));
      logParts.push("\n- Percent-like tokens:");
