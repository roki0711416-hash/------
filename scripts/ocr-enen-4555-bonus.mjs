...
const MACHINE_ID = 4555;
const IMAGE_URL = `https://p-town.dmm.com/upload/machines/${MACHINE_ID}/bonus.jpg`;

const TMP_DIR = path.join(process.cwd(), "scripts", ".tmp");
const IMAGE_PATH = path.join(TMP_DIR, `enen_${MACHINE_ID}_bonus.jpg`);
const UPSCALED_IMAGE_PATH = path.join(TMP_DIR, `enen_${MACHINE_ID}_bonus_upscaled.jpg`);
const OUT_PATH = path.join(TMP_DIR, `ocr_enen_${MACHINE_ID}_bonus.txt`);
...
async function main() {
  ...
  try {
    await execFileAsync("sips", ["-Z", "2600", IMAGE_PATH, "--out", UPSCALED_IMAGE_PATH]);
  ...
  // Pass 1: digits-only OCR (denominators / payout %)
  ...
  try {
    ...
    for (const psm of psms) {
      ...
      logParts.push("\n- Lines (filtered):");
      logParts.push(lines.slice(0, 320).join("\n"));
      logParts.push("\n- Numeric-like tokens (first 520):");
      logParts.push(numericLikeTokens.slice(0, 520).join(", "));
      logParts.push("\n- Percent-like tokens:");
  ...
  // Pass 2: Japanese OCR (headers like BIG/REG/合算/機械割 etc)
