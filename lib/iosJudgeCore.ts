...
export type JudgeSettingsResult = {
  posteriors: SettingPosterior[];
  top: SettingPosterior[];
};
...
export function topNSettings(posteriors: readonly SettingPosterior[], n: number): SettingPosterior[] {
  return [...posteriors]
    .sort((a, b) => b.posterior - a.posterior)
    .slice(0, Math.max(0, n));
}

export function judgeSettings(args: {
  settings: readonly MachineSettingOdds[];
  input: JudgeInput;
  options?: JudgeOptions;
  topN?: number;
}): JudgeSettingsResult {
  const posteriors = calcSettingPosteriors(args.settings, args.input, args.options);
  const top = topNSettings(posteriors, args.topN ?? 3);
  return { posteriors, top };
}
