...
import CommunityScopeJumpForm from "../../../components/CommunityScopeJumpForm";
...
export default async function CommunityBoardPage({
  ...
  const isPrefBoard = needsPrefecture(boardId);
  const isHallBoard = needsHall(boardId);
  const isMachineBoard = needsMachine(boardId);

  if (isPrefBoard || isHallBoard || isMachineBoard) {
    const machineMakers = isMachineBoard ? (await getMachinesData()).makers : undefined;
    ...
    return (
      <main className="mx-auto w-full max-w-xl px-4 pb-10 pt-6">
        <section className="rounded-2xl border border-neutral-200 bg-white p-5">
          ...
          <div className="mt-4 rounded-lg border border-neutral-200 bg-neutral-50 p-3">
            <p className="text-sm font-semibold text-neutral-800">条件を選んでください</p>
            <p className="mt-1 text-sm text-neutral-700">選択後にスレ一覧が表示され、そこからスレを立てられます。</p>
          </div>
          ...
          {isPrefBoard ? (
            <ul className="mt-4 grid grid-cols-2 gap-2">
              {PREFECTURES.map((p) => (
                <li key={p}>
                  <Link
                    href={`/community/${boardId}/pref/${encodeURIComponent(p)}`}
                    className="block rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-neutral-900"
                  >
                    {p}
                  </Link>
                </li>
          ...
          {isHallBoard ? <CommunityScopeJumpForm boardId={boardId} kind="hall" /> : null}
          {isMachineBoard ? <CommunityScopeJumpForm boardId={boardId} kind="machine" machineMakers={machineMakers} /> : null}
        </section>
      </main>
    );
  }
    ...
    <main className="mx-auto w-full max-w-xl px-4 pb-10 pt-6">
      <section className="rounded-2xl border border-neutral-200 bg-white p-5">
        ...
        <div id="new-thread" className="scroll-mt-24">
          <CommunityThreadForm boardId={boardId} username={username} />
        </div>
                ...
                return (
                  <li key={th.id}>
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-neutral-500">{th.username}{extra ? `・${extra}` : ""}</p>
                    </Link>
                  </li>
                );
              })}
