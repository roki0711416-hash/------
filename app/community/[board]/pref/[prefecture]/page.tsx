...
export default async function CommunityPrefecturePage({
  params,
}: {
  params: Promise<{ board: string; prefecture: string }>;
}) {
  const { board, prefecture } = await params;
  if (!isBoardId(board)) notFound();
  const boardId = board as BoardId;
  if (!needsPrefecture(boardId)) notFound();

  const pref = decodeURIComponent(prefecture);
  if (!(PREFECTURES as readonly string[]).includes(pref)) notFound();
  ...
  try {
    const result = await db.sql`
      ...
      WHERE board_id = ${boardId} AND prefecture = ${pref}
    ...
    <main className="mx-auto w-full max-w-xl px-4 pb-10 pt-6">
      <section className="rounded-2xl border border-neutral-200 bg-white p-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold">{boardLabel(boardId)}</h1>
            <p className="mt-1 text-sm font-semibold text-neutral-700">{pref}</p>
          </div>
          <Link
            href={`/community/${boardId}`}
            className="rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm font-medium"
        ...
        <div id="new-thread" className="scroll-mt-24">
          <CommunityThreadForm boardId={boardId} username={username} preset={{ prefecture: pref }} />
        </div>
