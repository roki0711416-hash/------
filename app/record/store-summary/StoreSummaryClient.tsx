    ...
    <main className="mx-auto w-full max-w-xl px-4 pb-10 pt-6">
      <header className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-5">
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-lg font-semibold text-white">店舗サマリ</h1>
          <Link
            href="/record"
        ...
        <p className="mt-2 text-sm text-muted">店舗ごとの台番Top5と実戦履歴を表示します。</p>
      </header>

      {!selectedShopName ? (
        <section className="mt-4 rounded-2xl border border-white/[0.08] bg-white/[0.04] p-5">
          <p className="text-sm text-muted">店舗が選択されていません。</p>
          <Link
            href="/record"
            className="mt-3 inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-cta-from to-cta-to px-4 py-3 text-sm font-semibold text-white"
          >
            収支管理へ戻る
          </Link>
        </section>
        ...
        <>
          <section className="mt-4 rounded-2xl border border-white/[0.08] bg-white/[0.04] p-5">
            ...
            <p className="mt-1 text-base font-semibold text-white">{selectedShopName}</p>
          </section>
          ...
          <section className="mt-4 space-y-3">
            {(
              ...
              <div key={card.title} className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-5">
                    ...
                    <table className="w-full min-w-[520px] border-collapse text-xs">
                      <thead>
                        <tr className="text-left text-muted">
                          <th className="border border-white/[0.08] px-3 py-2">台番</th>
                          <th className="border border-white/[0.08] px-3 py-2 text-right">回数</th>
                          <th className="border border-white/[0.08] px-3 py-2 text-right">累計</th>
                          <th className="border border-white/[0.08] px-3 py-2 text-right">平均</th>
                          <th className="border border-white/[0.08] px-3 py-2 text-right">最終日</th>
                        </tr>
                      </thead>
                      <tbody>
                        {card.rows.map((r) => {
                          const profitClass = r.totalProfit < 0 ? "text-red-600" : "text-white";
                          return (
                            <tr key={`${r.shopName}__${r.machineNumber}`} className="bg-white/[0.04]">
                              <td className="border border-white/[0.08] px-3 py-2 font-semibold text-white">
                                {r.machineNumber}
                              </td>
                              <td className="border border-white/[0.08] px-3 py-2 text-right text-white">
                                {r.plays}
                              </td>
                              <td className={`border border-white/[0.08] px-3 py-2 text-right font-semibold ${profitClass}`}>
            ...
            {storeEntries.length === 0 ? (
              ...
              <div className="mt-3 space-y-2">
                      ...
                      <div key={s.id} className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-3">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 truncate text-xs text-muted">{s.machineName}</p>
                            {s.memo ? (
                          ...
                          <div className="shrink-0 text-right">
                            ...
                            <p className="mt-1 text-xs font-semibold text-muted">差枚 {formatSignedNumber(s.diffCoins)}</p>
                          </div>
                        </div>
                      </div>
                    );
                ...
                {storeEntries.length > 50 ? (
                  <p className="pt-1 text-xs text-muted">表示は最新50件までです。</p>
                ) : null}
              </div>
            )}
          </section>
